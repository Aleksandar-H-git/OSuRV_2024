...
//                 printf("Sending pkg[0]=%d, pkg[1]=%d\n", pkg[0], pkg[1]);
//                 fflush(stdout); // osiguraj da se odmah ispiše
    ...
    while (1) {
        ...
        if (ready) {
            if (js_event_data.type & JS_EVENT_BUTTON && js_event_data.value == 1) {
                ...
                char buffer[10];
                snprintf(buffer, sizeof(buffer), "%d", js_event_data.number);
                printf("Sending: %s\n", buffer);
                fflush(stdout); // Ensure immediate output
                if (zmq_send(pusher, buffer, strlen(buffer), 0) == -1) {
                    perror("Failed to send ZeroMQ message");
                }
            }
            ready = 0; // Reset for next event
